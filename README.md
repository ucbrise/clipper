...
* Clipper **improves throughput** and ensure **reliable millisecond latencies** by introducing adaptive batching, caching, and anytime straggler mitigation techniques.  *Clipper makes the infra-team less unhappy.*
...
We are group of researchers in the UC Berkeley [RISE Lab](https://rise.cs.berkeley.edu/) studying the fundamental challenges of taking machine learning to production.  In collaboration with leading industrial and research organizations ([sponsors](https://rise.cs.berkeley.edu/sponsors/)) we identified model deployment as one of the next big challenges to the wide-scale adoption of AI technologies.
