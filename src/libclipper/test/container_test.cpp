...
TEST(ContainerTests, ModelContainerAverageThroughputUpdatesCorrectlyFewSamples) {
...
TEST(ContainerTests, ModelContainerAverageThroughputUpdatesCorrectlyManySamples) {
  VersionedModelId model("test", 1);
  ModelContainer container(model, 0, InputType::Doubles);
  double avg_throughput_millis = 0;
  for(int i = 3; i < 103; i++) {
