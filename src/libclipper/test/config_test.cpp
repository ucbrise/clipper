...
TEST_F(ConfigTest, TestReadBeforeReady) {
  ...
  ASSERT_THROW(conf.get_default_threadpool_size(), std::logic_error);
}
...
TEST_F(ConfigTest, TestWriteAfterReady) {
  ...
  ASSERT_THROW(conf.set_default_threadpool_size(40), std::logic_error);
...
TEST_F(ConfigTest, TestReadManyThreads) {
  ...
  for (int i = 0; i < 100; ++i) {
    threads.push_back(std::thread([i]() {
      ...
      ASSERT_EQ(conf.get_default_threadpool_size(), 2);
      ASSERT_THROW(conf.set_redis_port(5555), std::logic_error);
      ASSERT_THROW(conf.set_default_threadpool_size(50), std::logic_error);
      ASSERT_THROW(conf.set_redis_address("new_address"), std::logic_error);
    }));
  }
