...
TEST(InputSerializationTests, ByteSerialization) {
  ...
  uint32_t* request_header_data = static_cast<uint32_t*>(std::get<0>(request_header_buf).get());
  size_t request_header_start = std::get<1>(request_header_buf);

  uint64_t* input_header_size_data = static_cast<uint64_t*>(std::get<0>(input_header_size_buf).get());
  size_t input_header_size_start = std::get<1>(input_header_size_buf);

  uint64_t* input_header_data = static_cast<uint64_t*>(std::get<0>(input_header_buf).get());
  ...
  for (uint64_t i = 0; i < num_inputs; ++i) {
    ...
    uint8_t* serialized_input_data = static_cast<uint8_t*>(std::get<0>(serialized_input).get());
...
TEST(InputSerializationTests, IntSerialization) {
  ...
  for (uint64_t i = 0; i < num_inputs; ++i) {
    ...
    ASSERT_EQ(serialized_input_size_bytes, input_size_bytes);
    ASSERT_EQ(parsed_input_size_bytes, input_size_bytes);
    int* serialized_input_data = static_cast<int*>(std::get<0>(serialized_input).get());
...
TEST(InputSerializationTests, FloatSerialization) {
  ...
  for (uint64_t i = 0; i < num_inputs; ++i) {
    ...
    ASSERT_EQ(serialized_input_size_bytes, input_size_bytes);
    ASSERT_EQ(parsed_input_size_bytes, input_size_bytes);
    float* serialized_input_data = static_cast<float*>(std::get<0>(serialized_input).get());
...
TEST(InputSerializationTests, DoubleSerialization) {
  ...
  for (uint64_t i = 0; i < num_inputs; ++i) {
    ...
    ASSERT_EQ(serialized_input_size_bytes, input_size_bytes);
    ASSERT_EQ(parsed_input_size_bytes, input_size_bytes);
    double* serialized_input_data = static_cast<double*>(std::get<0>(serialized_input).get());
...
TEST(InputSerializationTests, StringSerialization) {
  ...
  for (uint64_t i = 0; i < num_inputs; ++i) {
    ...
    ASSERT_EQ(serialized_input_size_bytes, input_size_bytes);
    ASSERT_EQ(parsed_input_size_bytes, input_size_bytes);
    char* serialized_input_data = static_cast<char*>(std::get<0>(serialized_input).get());
...
std::vector<std::pair<SharedPoolPtr<T>, size_t>> get_primitive_hash_items() {
  ...
  SharedPoolPtr<T> hash_item_1(static_cast<T*>(malloc(hash_item_size * sizeof(T))));
  SharedPoolPtr<T> hash_item_3(static_cast<T*>(malloc(hash_item_size * sizeof(T))));
  ...
  SharedPoolPtr<T> hash_item_2(static_cast<T*>(malloc(hash_item_size * sizeof(T))));
...
TEST(InputHashTests, IntVectorsHashCorrectly) {
  ...
  SharedPoolPtr<int> new_item_data(static_cast<int*>(malloc((new_item_size) * sizeof(int))));
...
TEST(InputHashTests, FloatVectorsHashCorrectly) {
  ...
  ASSERT_NE(FloatVector(first_item_data, 0, first_item_size - 1).hash(),
            FloatVector(second_item_data, 0, second_item_size).hash());
  size_t new_item_size = second_item_size + 1;
  SharedPoolPtr<float> new_item_data(static_cast<float*>(malloc((new_item_size) * sizeof(float))));
  memcpy(new_item_data.get(), second_item_data.get(), second_item_size);
  new_item_data.get()[new_item_size - 1] = 500;
  // Adding the element 500, which is not present in the first item, to the
  // second item
...
TEST(InputHashTests, DoubleVectorsHashCorrectly) {
  ...
  ASSERT_NE(DoubleVector(first_item_data, 0, first_item_size - 1).hash(),
            DoubleVector(second_item_data, 0, second_item_size).hash());
  size_t new_item_size = second_item_size + 1;
  SharedPoolPtr<double> new_item_data(static_cast<double*>(malloc((new_item_size) * sizeof(double))));
  memcpy(new_item_data.get(), second_item_data.get(), second_item_size);
  new_item_data.get()[new_item_size - 1] = 500;
  // Adding the element 500, which is not present in the first item, to the
  // second item
...
TEST(InputHashTests, ByteVectorsHashCorrectly) {
  ...
  ASSERT_NE(ByteVector(first_item_data, 0, first_item_size - 1).hash(),
            ByteVector(second_item_data, 0, second_item_size).hash());
  size_t new_item_size = second_item_size + 1;
  SharedPoolPtr<uint8_t> new_item_data(static_cast<uint8_t*>(malloc((new_item_size) * sizeof(uint8_t))));
  memcpy(new_item_data.get(), second_item_data.get(), second_item_size);
...
TEST(OutputDeserializationTests, PredictionResponseDeserialization) {
  ...
  char* response_output_data = static_cast<char*>(get_data(response_output).get());
  std::string parsed_response_output(response_output_data + response_output->start(),
                                     response_output_data + response_output->start() + response_output->size());
