...
class Tester {
 public:
  explicit Tester(const int num_containers) : rpc_(std::make_unique<rpc::RPCService>()),
                                              msg_id_to_container_map(std::unordered_map<int, int>()),
  ...
  ~Tester() {
    ...
    bench_completed_cv_.wait(l, [this]() { return test_completed_ == true; });
  }
    ...
    redis::subscribe_to_container_changes(
        ...
        [this](const std::string &key, const std::string &event_type) {
          if (event_type == "hset") {
            ...
            long curr_time_millis = std::chrono::duration_cast<std::chrono::millis>(
                std::chrono::system_clock::now().time_since_epoch()).count();
            containers_connected += 1;
            int container_id = container_info["zmq_connection_id"];
            ...
            std::unique_lock lock(container_maps_mutex);
  ...
  const int send_validation_message(int container_id) {
    std::vector<uint8_t> empty_msg;
    int msg_id = rpc_->send_message(empty_msg, container_id);
    std::unique_lock lock(container_maps_mutex);
    return msg_id;
  }
 ...
 private:
  ...
  void on_response_received(rpc::RPCResponse response) {
    ...
    if(container_valid_entry == container_validation_map_.end()) {
      ...
      std::vector<int> parsed_message_history(
          static_cast<int *>(msg_history_bytes.begin()), msg_history_bytes.size() / sizeof(int));
      container_rpc_protocol_valid = validate_rpc_protocol(rpc_message_history);
      ...
      container_validation_map_[container_id] = failed_result;
    }
  ...
  RPCValidationResult validate_rpc_protocol(std::vector<int>& rpc_message_history) {
    ...
    for(int i = 3; i < rpc_message_history.size(); i++) {
...
int main(int argc, char *argv[]) {
  ...
  tester.test_completed_cv_.wait(
      l, [&tester]() { return tester.test_completed_; });
