...
std::vector<std::shared_ptr<I>> get_parsed_inputs(
    std::vector<D>& data_buffer, const std::vector<int>& input_header,
    long input_content_size,
    std::function<std::shared_ptr<I>(std::vector<D>&, int, int)>
        construct_input) {
  ...
  for (int i = 2; i < num_splits + 2; i++) {
    ...
    std::shared_ptr<I> input =
        construct_input(data_buffer, prev_split, curr_split);
    inputs.push_back(input);
    prev_split = curr_split;
  }
  int typed_input_content_size =
      static_cast<int>(input_content_size / static_cast<long>(sizeof(D)));
  std::shared_ptr<I> tail_input =
      construct_input(data_buffer, prev_split, typed_input_content_size);
...
const std::vector<std::shared_ptr<ByteVector>> ByteVectorParser::get_inputs(
  ...
  return get_parsed_inputs(
      buffer_, input_header, input_content_size,
      std::function<std::shared_ptr<ByteVector>(std::vector<uint8_t>&, int,
                                                int)>(construct_input));
}

std::shared_ptr<ByteVector> ByteVectorParser::construct_input(
  ...
  input_data.insert(input_data.end(), std::make_move_iterator(begin),
...
const std::vector<std::shared_ptr<IntVector>> IntVectorParser::get_inputs(
  ...
  return get_parsed_inputs(
      buffer_, input_header, input_content_size,
      std::function<std::shared_ptr<IntVector>(std::vector<int>&, int, int)>(
          construct_input));
}

std::shared_ptr<IntVector> IntVectorParser::construct_input(
...
const std::vector<std::shared_ptr<FloatVector>> FloatVectorParser::get_inputs(
  ...
  return get_parsed_inputs(
      buffer_, input_header, input_content_size,
      std::function<std::shared_ptr<FloatVector>(std::vector<float>&, int,
                                                 int)>(construct_input));
}

std::shared_ptr<FloatVector> FloatVectorParser::construct_input(
  ...
  input_data.insert(input_data.end(), std::make_move_iterator(begin),
                    std::make_move_iterator(end));
  std::shared_ptr<FloatVector> input =
      std::make_shared<FloatVector>(input_data);
  return input;
}
...
const std::vector<std::shared_ptr<DoubleVector>> DoubleVectorParser::get_inputs(
  ...
  return get_parsed_inputs(
      buffer_, input_header, input_content_size,
      std::function<std::shared_ptr<DoubleVector>(std::vector<double>&, int,
                                                  int)>(construct_input));
}

std::shared_ptr<DoubleVector> DoubleVectorParser::construct_input(
    std::vector<double>& data_buffer, int data_start, int data_end) {
  std::vector<double> input_data;
  input_data.reserve(data_end - data_start);
  auto begin = std::next(data_buffer.begin(), data_start);
  auto end = std::next(data_buffer.begin(), data_end);
  input_data.insert(input_data.begin(), std::make_move_iterator(begin),
                    std::make_move_iterator(end));
  std::shared_ptr<DoubleVector> input =
      std::make_shared<DoubleVector>(input_data);
  return input;
}

std::vector<char>& SerializableStringParser::get_data_buffer(
    long min_size_bytes) {
  resize_if_necessary(buffer_, min_size_bytes);
  return buffer_;
}

const std::vector<std::shared_ptr<SerializableString>>
SerializableStringParser::get_inputs(const std::vector<int>& /*input_header*/,
                                     long input_content_size) {
  ...
  while (bytes_processed < input_content_size) {
    ...
    std::shared_ptr<SerializableString> input =
        std::make_shared<SerializableString>(input_str);
    inputs.push_back(input);
  }
  return inputs;
}
