...
std::vector<std::shared_ptr<clipper::Input>> get_primitive_inputs(clipper::InputType type,
  ...
  for (int num_inputs = 0; num_inputs < 500; ++num_inputs) {
    ...
    generic_input_vector.push_back(std::dynamic_pointer_cast<clipper::Input>(input));
    data_vector.clear();
  }
  return generic_input_vector;
}

void run_bytes_benchmark(int container_id,
                         int num_iterations,
                         std::shared_ptr<clipper::rpc::RPCService> rpc_service,
                         std::shared_ptr<std::unordered_map<int, long>> times_map) {
  std::vector<uint8_t> type_vec;
  std::vector<std::shared_ptr<clipper::ByteVector>> input_vec;
  std::vector<std::shared_ptr<clipper::Input>>
      inputs = get_primitive_inputs(clipper::InputType::Bytes, type_vec, input_vec);
  clipper::rpc::PredictionRequest request(inputs, clipper::InputType::Bytes);
  send_request_repeated(container_id, num_iterations, request, rpc_service, times_map);
}

void run_ints_benchmark(int container_id,
                        int num_iterations,
                        std::shared_ptr<clipper::rpc::RPCService> rpc_service,
                        std::shared_ptr<std::unordered_map<int, long>> times_map) {
  ...
  std::vector<std::shared_ptr<clipper::Input>>
      inputs = get_primitive_inputs(clipper::InputType::Ints, type_vec, input_vec);
  clipper::rpc::PredictionRequest request(inputs, clipper::InputType::Ints);
  send_request_repeated(container_id, num_iterations, request, rpc_service, times_map);
}

void run_floats_benchmark(int container_id,
                          int num_iterations,
                          std::shared_ptr<clipper::rpc::RPCService> rpc_service,
                          std::shared_ptr<std::unordered_map<int, long>> times_map) {
  ...
  std::vector<std::shared_ptr<clipper::Input>>
      inputs = get_primitive_inputs(clipper::InputType::Floats, type_vec, input_vec);
  clipper::rpc::PredictionRequest request(inputs, clipper::InputType::Floats);
  send_request_repeated(container_id, num_iterations, request, rpc_service, times_map);
}

void run_doubles_benchmark(int container_id,
                           int num_iterations,
                           std::shared_ptr<clipper::rpc::RPCService> rpc_service,
                           std::shared_ptr<std::unordered_map<int, long>> times_map) {
  ...
  std::vector<std::shared_ptr<clipper::Input>>
      inputs = get_primitive_inputs(clipper::InputType::Doubles, type_vec, input_vec);
  clipper::rpc::PredictionRequest request(inputs, clipper::InputType::Doubles);
  send_request_repeated(container_id, num_iterations, request, rpc_service, times_map);
}

void run_strings_benchmark(int container_id,
                           int num_iterations,
                           std::shared_ptr<clipper::rpc::RPCService> rpc_service,
                           std::shared_ptr<std::unordered_map<int, long>> times_map) {
...
void run_benchmarks() {
  ...
  std::shared_ptr<clipper::rpc::RPCService> rpc_service = std::make_shared<clipper::rpc::RPCService>(containers);
  std::shared_ptr<std::unordered_map<int, long>> times_map = std::make_shared<std::unordered_map<int, long>>();
  bool shutdown = false;
  ...
  //run_bytes_benchmark(0, num_iterations_per_benchmark, rpc_service, times_map);
  //run_ints_benchmark(0, num_iterations_per_benchmark, rpc_service, times_map);
  //run_floats_benchmark(0, num_iterations_per_benchmark, rpc_service, times_map);
  run_doubles_benchmark(0, num_iterations_per_benchmark, rpc_service, times_map);
  //run_strings_benchmark(0, num_iterations_per_benchmark, rpc_service, times_map);

  shutdown = true;
  rpc_service->stop();
  printf("%f ms\n", ((double) total_time_elapsed) / (num_iterations_per_benchmark * num_benchmarks));
}
