...
std::vector<VersionedModelId> parse_model_json(
    ptree const& pt, ptree::key_type const& key) {
      ...
      try {
        ...
        if (!input_type_name.compare("double_vec")) {
          ...
          throw std::invalid_argument(
              input_type_name + " is not a valid input type");
        }
        ...
        if (!output_type_name.compare("double_val")) {
          ...
          throw std::invalid_argument(
              output_type_name + " is not a valid output type");
        }
