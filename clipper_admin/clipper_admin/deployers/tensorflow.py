    ...
    if isinstance(tf_sess_or_saved_model_path, Session):
        ...
        if os.path.isdir(tf_sess_or_saved_model_path):
            ...
            if (len(is_frozen_graph) > 0):
                ...
                if glob.glob(tf_sess_or_saved_model_path + "/*.meta") and glob.glob(
                    tf_sess_or_saved_model_path + "/*.index") and glob.glob(
                    tf_sess_or_saved_model_path + "/checkpoint") and glob.glob(
                    tf_sess_or_saved_model_path + "/*.data*"):
